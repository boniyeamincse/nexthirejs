...
**Overall Progress:** ~58% (weighted: 21/40 NH-M modules COMPLETED, 3 IN_PROGRESS, 0 UNVERIFIED, 16 NOT_STARTED — see `docs/task/status.md` ledger, reconciled 2026-07-23T10:45:00Z + NH-M09/NH-M10/NH-M12–NH-M14/NH-M16–NH-M22 work, treated as authoritative over `TODO.md`. NH-M07's COMPLETED status should be treated as unverified pending a routing sweep — see Bugs → Critical.)
...
- [x] Shortlists and Talent Pipeline (NH-M22) — new `TalentShortlist`/`TalentShortlistMember` models; each member row carries a fixed pipeline `stage` (SHORTLISTED→CONTACTED→SCREENING→INTERVIEWING→OFFER→HIRED→REJECTED, no custom stage names — same scope-reduction pattern as the rest of this phase), free-form notes/tags, and a per-(shortlist, stage) sortOrder. Reuses M21's `CompanyVerifiedAccessService` for tenant isolation and M21's `isDiscoverable()` check to gate additions — only platform-discoverable candidates can be shortlisted. OWNER/ADMIN/RECRUITER can write; VIEWER is read-only, enforced server-side. Movement has no drag-and-drop by design (the spec says "**accessible** pipeline board/list" and there's no DnD dependency in this repo) — moving a member re-splices an in-memory ordered ID list and reindexes 0..n in one transaction; the frontend exposes up/down buttons and a stage `<select>` instead, fully keyboard-operable. **Found and fixed a real packaging bug along the way**: `packages/constants/src/index.ts` re-exported `./companies/company-errors.js` directly instead of the `companies/index.ts` barrel (types/validation both correctly pointed at their barrels) — invisible until this module's second constants file in that folder silently never reached consumers; fixed to match the other two packages. Frontend: `/company/shortlists` (create + list), `/company/shortlists/[id]` (stage-grouped pipeline list with move/reorder/notes/tags/remove), an "Add to shortlist" widget on the M21 candidate detail page. 15 new backend unit tests + 8 new e2e smoke tests (same pre-existing harness break, not a regression) + 12 new frontend tests, all new.
...
## Company/Recruiter (NH-M19–M26) — profile/verification/team/candidate-search/shortlists done, rest not started
...
- [x] ~~Shortlists and talent pipeline~~ — done 2026-07-24, see Completed Features (M22). Fixed a lasting packaging bug in `@nexthire/constants`'s root barrel along the way. Candidate contact/email/SMS/consent is NH-M23 — tracked under Communication & Growth below, not here.
...
25. ~~Begin NH-M22 (Shortlists and Talent Pipeline)~~ — done 2026-07-24, see Completed Features. Found and fixed a real `@nexthire/constants` barrel-export bug along the way.
26. Begin NH-M23 (Candidate Contact, Email, SMS, and Consent) — next module of the Company/Recruiter phase.
