...
|    16 | NH-M16    | Feedback, Evaluation, Ratings, and Reviews         | COMPLETED   | Built 2026-07-23, on `ExpertBooking` (not legacy `trainers`/`Evaluation`, same domain-fork as M13/M14). Two new models: `ExpertSessionEvaluation` (expert's private feedback on the candidate's session, per spec's "Expert → Candidate evaluation") and `ExpertReview` (candidate's public rating/review of the expert — the aggregate-rating data source M13 explicitly deferred). One row per COMPLETED booking each way (unique on bookingId), eligibility enforced via booking ownership + COMPLETED status. `ExpertReview.isHidden/hiddenAt/hiddenReason` are the spec's "moderation hooks"; a `manage/experts/reviews` admin controller (hide/unhide) covers them — a full moderation center is NH-M32's scope, not built here. Wired real `rating: {average, count}` into `expert/public` list/detail (M13's placeholder), plus a new public `expert/public/:slug/reviews` endpoint (anonymized, non-hidden only). New endpoints: `candidates/me/bookings/:id/evaluation` (GET), `candidates/me/bookings/:id/review` (GET/POST), `expert/bookings/:id/evaluation` (GET/POST), `expert/bookings/:id/review` (GET), `expert/reviews` (GET, own aggregate). Frontend: feedback panels added to `/bookings` and `/expert/bookings` (expand-to-load, submit-once forms), new `/expert/reviews` page (added to expert nav), rating + reviews section added to `/find-expert` and `/find-expert/[slug]`. 24 new backend unit tests (evaluation + review services) + 4 new public-directory rating tests + 11 new e2e smoke tests (39/39 total in `experts.e2e-spec.ts`, was 28) + 10 new frontend tests. Commit `[NH-M16]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
|    17 | NH-M17    | Expert Earnings, Wallet, and Payout                | COMPLETED   | Built 2026-07-24, on a new `ExpertWalletAccount` keyed directly to `userId` (same domain-fork decision as M13/M14/M16 — not the legacy `WalletAccount`/`TrainerProfile` domain). New models: `ExpertWalletAccount`, `ExpertWalletLedger`, `ExpertPayoutAccount`, `ExpertPayoutRequest` (reusing the existing generic `WalletStatus`/`TransactionType`/`PayoutStatus` enums). Balances start and stay at real `0.00` — nothing auto-credits them, since no payment-capture domain (NH-M29) exists yet to source real earnings from; auto-crediting from booking completion would have repeated the exact "fabricated revenue" anti-pattern already flagged for `AdminDashboardService` (see Bugs → High). The ledger/debit mechanism is fully real and tested (`processPayoutRequest` debits the wallet + writes a `PAYOUT_OUT` ledger row on COMPLETED), just unreachable at nonzero balances until M29 lands. Security improvement over the legacy wallet: `accountNumber`/`routingNumber` are now masked (`maskTail`) in every API response — full values are never returned after creation. New expert endpoints (`expert/wallet` init/get, payout-accounts create/list, payout-requests create/list) and admin endpoints (`manage/experts/payout-requests` list/process, `manage/experts/payout-accounts/:id/verify` — the KYC gate blocking payout requests until an account is verified, same as legacy). Frontend: new `/expert/wallet` page (added to expert nav) — balance/earnings summary, payout-account form + masked list, payout-request form gated on having a verified account. 18 new backend unit tests + 9 new e2e smoke tests (48/48 total in `experts.e2e-spec.ts`, was 39) + 7 new frontend tests. Commit `[NH-M17]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
|    18 | NH-M18    | Expert Dashboard and Reports                       | COMPLETED   | Built 2026-07-24 as the capstone of the M10-M18 expert-marketplace arc — pure aggregation over models already built in M10-M17, no new Prisma models/migration needed. New `GET expert/dashboard` reuses `ExpertReviewService`/`ExpertWalletService` (via DI, not duplicated queries) alongside direct `ExpertBooking`/`ExpertService`/`ExpertAvailabilityProfile` counts, all in one `Promise.all`. Returns upcoming-bookings count + list (next 5, CONFIRMED and future), completed-sessions count, active-services count, rating aggregate, wallet summary (or null if not initialized), 3 most recent reviews (owner's view, includes hidden), and whether availability is configured yet. New `/expert/dashboard` frontend page (added as the first expert nav item) with stat tiles, an upcoming-sessions list, and wallet/reviews summary cards linking out to their full pages; an inline prompt links to `/expert/availability` when unconfigured. 2 new backend unit tests + 1 new e2e smoke test (49/49 total in `experts.e2e-spec.ts`, was 48) + 5 new frontend tests. Commit `[NH-M18]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
|    19 | NH-M19    | Company Profile and Verification                   | COMPLETED   | Built 2026-07-24. New `Company`/`CompanyVerificationApplication`/`CompanyVerificationDocument` models (one company per owner for now — team/multi-member support is NH-M20's scope), mirroring the NH-M10 expert-application architecture with scope adjustments: admin-role-gated review (`manage/companies/*`, `@RequireRoles('admin','super_admin')`) rather than a dedicated reviewer role, no `MfaRequiredGuard` on the review side (matches M16/M17 precedent), single consolidated onboarding page instead of a multi-page wizard. Same `DRAFT→SUBMITTED→UNDER_REVIEW→CHANGES_REQUESTED/APPROVED/REJECTED/WITHDRAWN` state machine as experts. Private verification documents (business registration, tax certificate, authorization letter, other) stored via HMAC-signed time-boxed URLs, magic-byte content verification, readiness gating (profile complete + business-registration doc present + MFA enabled) before submission is allowed. `company` role granted transactionally only on admin approval (`approveWithRoleAssignment`), same role-on-approval pattern as experts. Frontend: `/become-a-company` (profile form, document upload/list/remove, readiness blockers, submit/withdraw, rejection display), `/manage/companies/applications` (admin queue, status filter pills), `/manage/companies/applications/[applicationId]` (admin detail — company info, documents with signed-URL links, inline approve/reject/request-changes forms). 48 new backend unit tests (all passing), 17 new e2e smoke tests written in `companies.e2e-spec.ts`, 15 new frontend tests (all passing). Found but not fixed (out of scope): `getMyExpertProfile` in `api-client.ts` has a pre-existing (M10-era) response-shape bug — backend returns `{profile: X\|null}` but the frontend returns the raw body untyped as `ExpertProfileResult` without unwrapping `.profile`; the new `getMyCompanyProfile` was written correctly (does unwrap). **Also found, repo-wide, unrelated to M19:** the entire backend e2e suite (all 34 spec files, not just this module's) fails to load under `npx jest --config test/jest-e2e.json` with `SyntaxError: Cannot use 'import.meta' outside a module`, thrown from the generated Prisma client (`src/generated/prisma/client.ts`, generator boilerplate for locating the query-engine binary). Root cause: a `moduleNameMapper` entry that used to redirect the generated client to a hand-written stub (`src/__mocks__/prisma-client.mock.ts`, still present but now orphaned) was deleted 61 commits before NH-M14 even started, in `04bf084 feat(candidates): implement candidate location and career preferences (NH-P1-T007)`, and replaced with unrelated `@nexthire/*` mappers — so this predates the entire M14-M19 arc. Confirmed a real Postgres is reachable on `localhost:5434` (the configured `DATABASE_URL`), so these e2e specs are meant to run against a live DB, not a mock — restoring the old stub mapping was tried and rejected (it turned the single import-time crash into 327 new failures, since the stub has no model delegates like `.userRole.deleteMany()` that many specs call). The real fix is a Jest ESM migration (`extensionsToTreatAsEsm`, `NODE_OPTIONS=--experimental-vm-modules`, `ts-jest` ESM preset) — out of scope for this module, left as a separate follow-up. Backend unit tests, typecheck, and production builds (both apps) are all clean/unaffected by this; the 48 new NH-M19 unit tests are the verification backstop in its place. Commit `[NH-M19]`                                                                                                                                                                                                                                                                                                                                                                                                      |
|    20 | NH-M20    | Company Team and Permissions                       | COMPLETED   | Built 2026-07-24. New `CompanyMember` (one row per company+user, role `OWNER`/`ADMIN`/`RECRUITER`/`VIEWER`) and `CompanyInvitation` (`PENDING`/`ACCEPTED`/`DECLINED`/`REVOKED`/`EXPIRED`) models. `Company.upsert` (NH-M19's bootstrap path) now transactionally creates the owner's `CompanyMember(OWNER)` row alongside the Company itself — the single seed point for all membership. Invitations target existing NextHire user accounts by email only (no invite-to-signup flow, no email delivery integration — invitee sees it via a `GET companies/invitations/me` list, matched by their own authenticated email; documented limitation, not a bug). No bearer-token accept link: authorization is by unguessable UUID + email-match, not a separate secret (simplified out of an initial `tokenHash` design mid-build, via a corrective migration, before anything used it). Permission model: OWNER manages everyone; ADMIN can invite/manage RECRUITER/VIEWER only (not other ADMINs, not OWNER); OWNER can never be changed, removed, or invited into (ownership transfer is explicitly out of scope); any non-OWNER member may remove themselves (leave). New endpoints: `companies/me/team` (GET roster, GET role), `companies/me/team/members/:id` (PATCH role, DELETE), `companies/me/team/invitations` (GET/POST, DELETE to revoke), `companies/invitations/me` (GET mine), `companies/invitations/:id/accept`\|`/decline`. **Scope decision:** M19's owner-only profile/application endpoints were deliberately NOT retrofitted to be membership-aware in this module — doing so would require reworking whose MFA/identity counts for readiness checks when a non-owner ADMIN acts (the readiness service takes an `ownerUserId`, not an acting-member id), which risked destabilizing already-tested M19 code for a bounded module; deferred as a documented follow-up. "Tenant isolation" for M20 itself means every team/invitation query is scoped by the caller's own `companyId` derived from their own membership row — cross-tenant access is structurally impossible, verified by tests. Frontend: `/company/team` (roster, role changes, remove/leave, invite form + pending-invitation list for managers), `/company/invitations` (accept/decline invites addressed to me), a "Manage team →" link added to `/become-a-company`. 28 new backend unit tests (all passing), 10 new e2e auth-boundary smoke tests added to `companies.e2e-spec.ts` (written, confirmed hitting the same pre-existing e2e-harness break documented under NH-M19 — not a new regression), 11 new frontend tests (all passing). Commit `[NH-M20]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
|    21 | NH-M21    | Company Candidate Search                           | COMPLETED   | Built 2026-07-24. Reuses NH-M09's privacy infrastructure rather than duplicating it: added `'COMPANY_VERIFIED'` to `CandidatePrivacyDecisionService`'s `ViewerContext` union (2-line additive change — same discoverability rule as `PLATFORM_AUTHENTICATED`, kept as its own branch for audit/semantic clarity and future divergence, since a payment/contact-consent module (NH-M27) will likely need companies to have _stricter_ access than generic platform peers). New `CandidateProfilePreviewService.getProfileForVerifiedCompany(companyUserId, candidateUserId)` mirrors the existing `getExternalProfileByPublicId` exactly but with real audit attribution (`actorType: USER`, not `ANONYMOUS`) since the viewer is now an identified company member. `PublicCandidateProfile` never included email/phone in the first place (confirmed by reading the full projection before building on it) — so there was no risk of a company search leaking contact info regardless of viewer-context wiring. New `CandidateSearchRepository`/`CandidateSearchService` (candidates module, exported) query `User` joined through `profilePrivacy`/`candidateProfile`/`candidatePreference`/`skills` relations, filtered to `overallDiscoverability=PLATFORM_DISCOVERABLE` + `basicProfile≠HIDDEN`; per-card location/skills fields are independently gated through the same `canExternalViewerReadSection` used everywhere else (no reimplemented privacy logic). New `CompanyVerifiedAccessService` (companies module) requires company membership (any role) **and** an `APPROVED` verification application — verification status has no denormalized flag on `Company`, checked by a new `CompanyApplicationRepository.isApproved(companyId)` count query. **CV authorization, scoped down deliberately**: `Cv.visibility` (`PRIVATE`/`UNLISTED`/`PUBLIC`) has existed since NH-M06 with a working owner-facing selector in `CvEditor` but zero consumers anywhere — new `CvService.listPublicCvsForCandidate()` (additive method, doesn't touch any owner-gated code) gives it its first real one: verified companies see metadata (title/template/completion%/updated) for a candidate's `PUBLIC` CVs only. Full CV content/PDF viewing for companies was **not** built — that would mean extending the owner-gated async export pipeline (BullMQ + signed URLs) with a second authorization branch, risking already-tested NH-M06 code for a module that's supposed to stay bounded; left as a documented follow-up, same call as M19→M20's profile/application boundary. New endpoints: `GET companies/me/candidates` (search), `GET companies/me/candidates/:candidateId` (profile + public-CV list). Frontend: `/company/candidates` (search bar, skill filter, card list — fixed a state-management bug caught by its own test before commit, where typing in the search box was auto-refetching on every keystroke because the live input state fed directly into the data-loading callback's dependency array; split into separate input-vs-applied-query state), `/company/candidates/[candidateId]` (privacy-projected profile view + public-CV list), a "Candidate search →" link added to `/company/team`. 7 new backend unit tests + 2 new e2e auth-boundary smoke tests (hit the same pre-existing e2e-harness break documented under NH-M19, not a new regression) + 9 new frontend tests, all passing. Commit `[NH-M21]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
|    22 | NH-M22    | Shortlists and Talent Pipeline                     | COMPLETED   | Built 2026-07-24. New `TalentShortlist`/`TalentShortlistMember` models — a company can have multiple named shortlists, each candidate row carries a fixed pipeline `stage` (`SHORTLISTED→CONTACTED→SCREENING→INTERVIEWING→OFFER→HIRED→REJECTED`, no custom per-company stage names — deliberate scope reduction, same pattern as every other module this phase), free-form `notes`/`tags`, and a per-(shortlist, stage) `sortOrder`. Reuses NH-M21's `CompanyVerifiedAccessService` for tenant isolation and NH-M21's `CandidateSearchService.isDiscoverable()` to gate additions — only platform-discoverable candidates can be shortlisted, same boundary as candidate search itself. Role gating: OWNER/ADMIN/RECRUITER can create/edit/manage; VIEWER is read-only (enforced server-side, not just hidden in the UI). Movement/reordering has no drag-and-drop: moving a member (stage change and/or position change) removes it from an in-memory ordered list of its destination stage's siblings, re-splices it at the target index, and reindexes 0..n in one transaction — deliberately chosen over drag-and-drop specifically because the spec calls out "**accessible** pipeline board/list" and this repo has no DnD dependency; the frontend instead exposes explicit up/down buttons and a stage `<select>`, fully keyboard/screen-reader operable. New endpoints under `companies/me/shortlists` (CRUD + `/members` add/update/remove). Frontend: `/company/shortlists` (create + list with member counts), `/company/shortlists/[id]` (stage-grouped member list — move/reorder/notes/tags/remove), an "Add to shortlist" widget added to the NH-M21 candidate detail page. **Found and fixed one packaging bug along the way**: `packages/constants/src/index.ts` re-exported `./companies/company-errors.js` directly instead of the `companies/index.ts` barrel (unlike the `types`/`validation` packages, which both correctly pointed at their barrels) — invisible until this module added a second file (`talent-pipeline.ts`) to the `companies/` folder in constants, at which point its exports silently never reached consumers; fixed by pointing the root barrel at `./companies/index.js` like the other two packages. 15 new backend unit tests + 8 new e2e auth-boundary smoke tests (hit the same pre-existing e2e-harness break documented under NH-M19, not a new regression) + 12 new frontend tests, all passing. Commit `[NH-M22]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
