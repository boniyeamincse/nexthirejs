...
export default function EditServicePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [service, areas] = await Promise.all([
        getExpertService(token, serviceId),
        getExpertiseAreas(),
      ]);
      if (service.status !== 'DRAFT' && service.status !== 'INACTIVE') {
        setNotEditable(true);
        setLoading(false);
        return;
      }
      setExpertiseAreaId(service.expertiseAreaId);
      setType(service.type);
      setTitle(service.title);
      setShortDescription(service.shortDescription);
      setDetailedDescription(service.detailedDescription);
      setDurationMinutes(service.durationMinutes);
      setPriceAmount(service.price.amount);
      setPriceCurrency(service.price.currency);
      setLanguageCodes(service.languageCodes.join(', '));
      setPreparationInstructions(service.preparationInstructions ?? '');
      ...
      setLoadError('Failed to load service data. Please try again.');
    } finally {
      setLoading(false);
    }
  }, [getAccessToken, logout, serviceId]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await updateExpertService(token, serviceId, {
        expertiseAreaId,
        type,
        title: title.trim(),
      ...
      setSaveError(err instanceof Error ? err.message : 'Failed to update service.');
    } finally {
      setSaving(false);
    }
  }
  ...
  if (notEditable) {
    return (
      <div>
        <p style={{ color: '#fca5a5' }}>This service cannot be edited in its current status.</p>
        <button
  ...
  return (
    <div>
      ...
      <p style={{ color: '#94a3b8', margin: '0 0 1.5rem' }}>Update your expert service offering.</p>
