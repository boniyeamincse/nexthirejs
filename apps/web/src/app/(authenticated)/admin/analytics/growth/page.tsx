...
import { getAdminGrowthUsers } from '@/lib/api-client';
...
export default function GrowthAnalyticsPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (!token) return;
      try {
        const data = await getAdminGrowthUsers(token);
        setGrowth(data.growth);
      } catch (err: any) {
        setErrorMsg(err.message || 'Failed to load growth analytics');
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className={dashboardStyles.page}>
      ...
      <div className={dashboardStyles.container}>
        <div className={dashboardStyles.hero}>
          ...
          <h1 className={dashboardStyles.greeting}>Growth Analytics</h1>
        ...
        <div className={dashboardStyles.card} style={{ marginBottom: '2rem' }}>
          <div className={dashboardStyles.cardHeader}>
            <h2 className={dashboardStyles.cardTitle}>User Growth (30 Days)</h2>
          </div>
            ...
            {loading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={growth} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                  ...
                  <Line type="monotone" dataKey="users" stroke="#6366f1" strokeWidth={3} dot={{ r: 4, fill: '#6366f1', strokeWidth: 2, stroke: '#1e293b' }} />
                </LineChart>
              </ResponsiveContainer>
            )}
          </div>
