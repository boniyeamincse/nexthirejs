...
export default function ExperiencePage() {
  ...
  const fetchRecords = useCallback(
    async (signal?: AbortSignal) => {
      ...
      try {
        ...
        if (!signal?.aborted) {
          setErrorMsg(
            err instanceof Error ? err.message : 'Failed to load work experience records',
          );
        }
      }
    },
  ...
  const handleSave = useCallback(
    async (data: CreateWorkExperienceRecordInput | UpdateWorkExperienceRecordInput) => {
      const token = getAccessToken();
      if (!token) throw new Error('Not authenticated');

      if (editingRecord) {
        const result = await updateWorkExperienceRecord(
          token,
          editingRecord.id,
          data as UpdateWorkExperienceRecordInput,
        );
        setRecords((prev) => prev.map((r) => (r.id === result.record.id ? result.record : r)));
        setCompletion(result.completion);
      } else {
        const result = await createWorkExperienceRecord(
          token,
          data as CreateWorkExperienceRecordInput,
        );
