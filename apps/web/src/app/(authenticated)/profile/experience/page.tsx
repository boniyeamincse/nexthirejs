...
import {
  ...
  reorderWorkExperienceRecords,
} from '@/lib/api-client';
import type { WorkExperienceRecordResult, CandidateProfileCompletion } from '@nexthire/types';
import type { CreateWorkExperienceRecordInput, UpdateWorkExperienceRecordInput } from '@nexthire/validation';
import styles from '@/app/(auth)/auth.module.css';
import { WorkExperienceList } from '@/features/candidate-profile/experience/WorkExperienceList';
import { WorkExperienceForm } from '@/features/candidate-profile/experience/WorkExperienceForm';

export default function ExperiencePage() {
  const { getAccessToken } = useAuth();

  const [loading, setLoading] = useState(true);
  const [records, setRecords] = useState<WorkExperienceRecordResult[]>([]);
  ...
  const handleSave = useCallback(async (data: CreateWorkExperienceRecordInput | UpdateWorkExperienceRecordInput) => {
    ...
    if (editingRecord) {
      const result = await updateWorkExperienceRecord(token, editingRecord.id, data as UpdateWorkExperienceRecordInput);
      setRecords((prev) => prev.map((r) => (r.id === result.record.id ? result.record : r)));
      setCompletion(result.completion);
    } else {
      const result = await createWorkExperienceRecord(token, data as CreateWorkExperienceRecordInput);
  ...
  const handleDelete = useCallback(async (id: string) => {
    ...
    try {
      await deleteWorkExperienceRecord(token, id);
    ...
    return (
      <div className={styles.container}>
        ...
        <div className={styles.glassCard} style={{ textAlign: 'center', padding: '4rem' }}>
          <p className={styles.subtitle}>Loading work experience records...</p>
        </div>
      </div>
    );
  }
    ...
    <div className={styles.container} style={{ minHeight: 'calc(100vh - 72px)', padding: '2rem 1rem' }}>
      ...
      <div className={styles.glassCard} style={{ maxWidth: '800px', margin: '0 auto' }}>
        <div
          ...
          style={{ marginBottom: '2rem', textAlign: 'left', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
        >
          <div>
            ...
            <p className={styles.subtitle}>Add your professional work history.</p>
          </div>
        ...
        {!isFormOpen && completion && (
          ...
          >
            ...
            <div style={{ width: '100%', height: '8px', background: 'rgba(255,255,255,0.1)', borderRadius: '4px', overflow: 'hidden' }}>
              <div
                style={{ width: `${completion.percentage}%`, height: '100%', background: 'linear-gradient(90deg, #6366f1, #a5b4fc)', transition: 'width 0.5s ease' }}
              ></div>
            </div>
          </div>
        )}
