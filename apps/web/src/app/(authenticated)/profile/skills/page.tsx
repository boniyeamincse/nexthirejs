...
import { listMySkillRecords, createSkillRecord, updateSkillRecord, deleteSkillRecord, reorderSkillRecords } from '@/lib/api-client';
...
export default function SkillsPage() {
  ...
  const handleSave = useCallback(async (data: CreateCandidateSkillInput | UpdateCandidateSkillInput) => {
    const token = getAccessToken();
    if (!token) throw new Error('Not authenticated');

    if (editingRecord) {
      const result = await updateSkillRecord(token, editingRecord.id, data as UpdateCandidateSkillInput);
      ...
      <div className={styles.glassCard} style={{ maxWidth: '800px', margin: '0 auto' }}>
        <div className={styles.header} style={{ marginBottom: '2rem', textAlign: 'left', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <h1 className={styles.title} style={{ fontSize: '2rem' }}>Skills</h1>
