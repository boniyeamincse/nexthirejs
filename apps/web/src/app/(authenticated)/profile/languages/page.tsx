...
import { listMyLanguageRecords, createLanguageRecord, updateLanguageRecord, deleteLanguageRecord, reorderLanguageRecords } from '@/lib/api-client';
import type { CandidateProfileCompletion, CandidateLanguageResult } from '@nexthire/types';
...
export default function LanguagesPage() {
  ...
  const [records, setRecords] = useState<CandidateLanguageResult[]>([]);
  const [completion, setCompletion] = useState<CandidateProfileCompletion | null>(null);

  const [isFormOpen, setIsFormOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<CandidateLanguageResult | null>(null);
  ...
  const fetchRecords = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      const data = await listMyLanguageRecords(token);
      if (!signal?.aborted) {
      ...
      if (!signal?.aborted) {
        setErrorMsg(err instanceof Error ? err.message : 'Failed to load languages');
      }
    }
  }, [getAccessToken]);
  ...
  const handleEdit = useCallback((record: CandidateLanguageResult) => {
  ...
  const handleSave = useCallback(async (data: CreateCandidateLanguageInput | UpdateCandidateLanguageInput) => {
    const token = getAccessToken();
    if (!token) throw new Error('Not authenticated');

    if (editingRecord) {
      const result = await updateLanguageRecord(token, editingRecord.id, data as UpdateCandidateLanguageInput);
      setRecords((prev) => prev.map((r) => (r.id === result.record.id ? result.record : r)));
      setCompletion(result.completion);
    } else {
      const result = await createLanguageRecord(token, data as CreateCandidateLanguageInput);
  ...
  const handleDelete = useCallback(async (id: string) => {
    ...
    try {
      await deleteLanguageRecord(token, id);
      setRecords((prev) => prev.filter((r) => r.id !== id));
      // Reload to get updated completion
      await fetchRecords();
    } catch (err: unknown) {
      setErrorMsg(err instanceof Error ? err.message : 'Failed to delete language');
    }
  ...
  const handleReorder = useCallback(async (startIndex: number, endIndex: number) => {
    ...
    try {
      ...
      await reorderLanguageRecords(token, { orderedIds });
    } catch (err: unknown) {
      setErrorMsg(err instanceof Error ? err.message : 'Failed to reorder languages');
      void fetchRecords();
    }
    ...
    return (
      <div className={styles.container}>
        ...
        <div className={styles.glassCard} style={{ textAlign: 'center', padding: '4rem' }}>
          <p className={styles.subtitle}>Loading languages...</p>
        </div>
      </div>
    );
  }
      ...
      <div className={styles.glassCard} style={{ maxWidth: '800px', margin: '0 auto' }}>
        <div className={styles.header} style={{ marginBottom: '2rem', textAlign: 'left', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <h1 className={styles.title} style={{ fontSize: '2rem' }}>Languages</h1>
            <p className={styles.subtitle}>Add languages you speak and your proficiency levels.</p>
          </div>
