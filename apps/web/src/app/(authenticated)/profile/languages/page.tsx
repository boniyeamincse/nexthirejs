...
export default function LanguagesPage() {
  ...
  const handleSave = useCallback(
    async (data: CreateCandidateLanguageInput | UpdateCandidateLanguageInput) => {
      const token = getAccessToken();
      if (!token) throw new Error('Not authenticated');

      if (editingRecord) {
        const result = await updateLanguageRecord(
          token,
          editingRecord.id,
          data as UpdateCandidateLanguageInput,
        );
  ...
  return (
    ...
    >
      ...
      <div className={styles.glassCard} style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        {isFormOpen ? (
          <LanguageForm initialData={editingRecord} onSave={handleSave} onCancel={handleCancel} />
        ) : (
          <LanguageList
            records={records}
            onEdit={handleEdit}
