...
export class CandidateLanguageService {
  private readonly logger = new Logger(CandidateLanguageService.name);
  private readonly MAX_LANGUAGES = 20;
  ...
  async listRecords(userId: string) {
    try {
      ...
      await this.auditService.recordRequired({
        ...
        actorUserId: userId,
        action: 'candidate.language.viewed',
  ...
  async createRecord(userId: string, input: any) {
    try {
      const parseResult = candidateLanguageSchema.safeParse(input);
      if (!parseResult.success) {
        throw new BadRequestException('CANDIDATE_LANGUAGE_VALIDATION_FAILED');
      }
      ...
      const count = await this.languageRepository.countByUserId(userId);
      if (count >= this.MAX_LANGUAGES) {
        throw new BadRequestException('CANDIDATE_LANGUAGE_LIMIT_REACHED');
      }
      ...
      await this.auditService.recordRequired({
        actorType: AuditActorType.USER,
        actorUserId: userId,
        action: 'candidate.language.created',
        targetType: 'CandidateLanguage',
        targetId: record.id,
        outcome: AuditOutcome.SUCCESS,
        metadata: {
          recordId: record.id,
  ...
  async updateRecord(userId: string, recordId: string, input: any) {
    try {
      const parseResult = updateCandidateLanguageSchema.safeParse(input);
      if (!parseResult.success) {
        throw new BadRequestException('CANDIDATE_LANGUAGE_VALIDATION_FAILED');
      }
      const validatedData = parseResult.data;

      const existing = await this.languageRepository.findByIdAndUserId(recordId, userId);
      if (!existing) {
        throw new NotFoundException('CANDIDATE_LANGUAGE_NOT_FOUND');
      }
      ...
      await this.auditService.recordRequired({
        actorType: AuditActorType.USER,
        actorUserId: userId,
        action: 'candidate.language.updated',
        targetType: 'CandidateLanguage',
        targetId: record.id,
        outcome: AuditOutcome.SUCCESS,
        metadata: {
  ...
  async deleteRecord(userId: string, recordId: string) {
    try {
      ...
      const countBefore = await this.languageRepository.countByUserId(userId);
      await this.languageRepository.delete(recordId);
      const completion = await this.recalculateCompletion(userId);

      await this.auditService.recordRequired({
        actorType: AuditActorType.USER,
        actorUserId: userId,
        action: 'candidate.language.deleted',
        targetType: 'CandidateLanguage',
        targetId: recordId,
        outcome: AuditOutcome.SUCCESS,
        metadata: {
  ...
  async reorderRecords(userId: string, input: any) {
    try {
      const parseResult = reorderCandidateLanguagesSchema.safeParse(input);
      if (!parseResult.success) {
        throw new BadRequestException('CANDIDATE_LANGUAGE_VALIDATION_FAILED');
      }
      const validatedData = parseResult.data;
      ...
      await this.languageRepository.updateSortOrder(updates);
      const completion = await this.recalculateCompletion(userId);

      await this.auditService.recordRequired({
        actorType: AuditActorType.USER,
        actorUserId: userId,
        action: 'candidate.language.reordered',
        targetType: 'CandidateProfile',
        targetId: userId,
        outcome: AuditOutcome.SUCCESS,
        metadata: {},
