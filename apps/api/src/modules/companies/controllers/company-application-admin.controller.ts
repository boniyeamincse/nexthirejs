...
@ApiTags('Company Application Review')
...
export class CompanyApplicationAdminController {
  constructor(
    private readonly reviewService: CompanyApplicationReviewService,
  ...
  @ApiOperation({ summary: 'List company applications in the review queue' })
  ...
  @Get('documents')
  @Throttle({ default: { limit: COMPANY_RATE_LIMITS.DOCUMENT_ACCESS_PER_HOUR, ttl: HOUR_MS } })
  ...
  async getDocument(
    ...
    if (!key || !expires || !signature) {
      throw new BadRequestException('COMPANY_SIGNED_URL_INVALID');
    }
  ...
  @ApiResponse({ status: 404, description: 'COMPANY_APPLICATION_NOT_FOUND' })
  ...
  @HttpCode(HttpStatus.OK)
  @Throttle({ default: { limit: COMPANY_RATE_LIMITS.REVIEW_DECISION_PER_HOUR, ttl: HOUR_MS } })
  @ApiOperation({ summary: 'Approve an application and grant the company role' })
  ...
  @ApiResponse({ status: 400, description: 'COMPANY_REVIEW_DECISION_INVALID' })
  async reject(
    @Req() req: AuthenticatedRequest,
    @Param('applicationId') applicationId: string,
    @Body() body: unknown,
