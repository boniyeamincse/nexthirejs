...
export class CertificateStorageService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.bucket = this.configService.get<string>('CERTIFICATE_STORAGE_BUCKET', 'nexthire-certificates');
    this.useS3 = this.configService.get<string>('CERTIFICATE_STORAGE_TYPE', 'local') === 's3';
    this.localPath = this.configService.get<string>('CERTIFICATE_LOCAL_PATH', './storage/certificates');
  ...
  async getPresignedUrl(key: string, expiresInSeconds = 300): Promise<string> {
    ...
    // Local filesystem: return a file path (the API will serve it)
    const filePath = path.join(this.localPath, key);
    if (!fs.existsSync(filePath)) {
      throw new Error(`Certificate file not found: ${key}`);
    }
    return `/api/v1/candidates/me/certificates/download/${path.basename(key)}`;
  }
  ...
  generateKey(certificateId: string): string {
    const random = randomBytes(16).toString('hex');
    return `certificates/${certificateId}/${random}.pdf`;
  }
}
